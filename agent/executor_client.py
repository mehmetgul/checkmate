...
class PlaywrightExecutorClient:
    ...
    async def execute_with_state_stream(
        self,
        base_url: str,
        steps: list[dict],
        test_id: Optional[str] = None,
        ...
        capture_state_after: bool = False,
    ) -> AsyncGenerator[dict, None]:
        """Execute test steps with state management via SSE.

        This endpoint supports Playwright-style fixtures by allowing:
        - Restoring browser state before execution (cookies, localStorage, sessionStorage)
        - Capturing browser state after execution for caching
        ...
        Args:
            ...
            test_id: Optional test identifier
            options: Optional execution options
            initial_state: Optional state to restore before execution
            capture_state_after: Whether to capture state after execution

        Yields:
            Event dicts from the execution stream, including:
            - state_restored: Initial state was restored
            - step_started/completed: Step execution events
            - state_captured: Browser state captured (if capture_state_after=True)
            - completed: Final result with optional captured_state
        """
        request_body = {
            "test_id": test_id,
            "base_url": base_url,
        ...
        try:
            async with self.client.stream(
                "POST",
                f"{self.base_url}/execute-with-state",
                json=request_body,
                headers=self._get_headers(),
                timeout=300.0,
            ) as response:
            ...
            logger.error(f"Playwright executor with state error: {e}")
            yield {
                "type": "error",
                "error": str(e),
            }
